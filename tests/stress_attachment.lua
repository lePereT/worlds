...
local W=require('worlds'); local Model,Att=W.Model,W.Att; local Ref=require('tests.reference.attachment')
local rounds=800
for n=1,rounds do
  ...
  local q=Att.at(m,K):query(Att.patch(m,gate),fixed); local st,w=q:step(math.huge)
  if #rs==0 then assert(st=='retry') else assert(st=='hit' and Att.is_witness(w)) end
  if #rs==1 and n%7==0 then local i=w:graft(); assert(m:is_realised(i.map[G])) end
end
