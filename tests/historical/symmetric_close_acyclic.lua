...
T.eq(#g:faces(),2); T.eq(#g:membranes(),1,'cut identifies process locality'); T.eq(images[1].points[x],images[2].points[y],'cut unifies provisional identity'); T.ok(g:is_input(images[1].strands[ai])); T.ok(not g:is_terminal(images[1].strands[ao])); T.ok(g:is_terminal(images[2].strands[bo]));
return T.count()
