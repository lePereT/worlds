...
do
  ...
  local q=Query.solve(Query.new(world,pat,{sources={ss[2]}})); local k,es=decided(q,10); eq(k,'yes'); eq(#es,1); eq(es[1].from,ss[2]); eq(es[1].to,d[1]); eq(decided(q,10),'done')
end

-- Target selection makes only that exact target part of the Question.
do
  local world,ss=token_world(1); local pat,d=pattern(2)
  local q=Query.solve(Query.new(world,pat,{targets={d[2]}})); local k,es=decided(q,10); eq(k,'yes'); eq(#es,1); eq(es[1].from,ss[1]); eq(es[1].to,d[2])
...
do
  local world,ss=token_world(1); local pat,d=pattern(2)
  local q=Query.solve(Query.new(world,pat,{targets=d,required={}})); local seen_empty,seen_one,hits=false,false,0
...
do
  ...
  local q=Query.solve(Query.new(world,pat,{admissible=admissible})); local k,es=decided(q,100); eq(k,'yes'); local by={}; for _,e in ipairs(es) do by[e.to]=e.from end; eq(by[d[1]],ss[2]); eq(by[d[2]],ss[1]); eq(decided(q,100),'done')
end

-- Section arrays denote sets: caller order cannot become priority.
do
  local world,ss=token_world(3); local pat,d=pattern(2)
  local function first(xs) local k,es=decided(Query.solve(Query.new(world,pat,{sources=xs})),100); eq(k,'yes'); return es end
...
do
  local world,ss=token_world(2); local pat,d=pattern(2)
  ok(not pcall(function() Query.new(world,pat,{sources={ss[1]},seeds={{from=ss[2],to=d[1]}}}) end))
  ok(not pcall(function() Query.new(world,pat,{targets={d[1]},seeds={{from=ss[1],to=d[2]}}}) end))
  ok(not pcall(function() Query.new(world,pat,{admissible={{from=ss[1],to=d[1]}},seeds={{from=ss[2],to=d[2]}}}) end))
end

-- An empty target section is the unit Question and yields one empty witness.
do
  local world=token_world(0); local pat,d=pattern(2)
  local q=Query.solve(Query.new(world,pat,{targets={}})); local k,es=decided(q,10); eq(k,'yes'); eq(#es,0); eq(decided(q,10),'done')
...
do
  ...
  local joined=pcall(function() return W.join({world,pat},{{from=s,to=d}}) end); ok(joined,'join should accept the materialisable quotient')
  local q=Query.new(world,pat,{seeds={{from=s,to=d}}})
...
do
  ...
  local question=Query.new(world,pat,{seeds={seed},admissible=admissible})
...
do
  local world,ss=token_world(2); local pat,d=pattern(2)
  ok(not pcall(function() Query.new(world,pat,{seeds={{from=ss[1],to=d[1]},{from=ss[1],to=d[1]}}}) end))
  ok(not pcall(function() Query.new(world,pat,{seeds={{from=ss[1],to=d[1]},{from=ss[1],to=d[2]}}}) end))
  ok(not pcall(function() Query.new(world,pat,{seeds={{from=ss[1],to=d[1]},{from=ss[2],to=d[1]}}}) end))
  local q1=Query.new(world,pat,{seeds={{from=ss[2],to=d[2]},{from=ss[1],to=d[1]}}})
  local q2=Query.new(world,pat,{seeds={{from=ss[1],to=d[1]},{from=ss[2],to=d[2]}}})
...
do
  ...
  ok(not pcall(function() Query.new(world,pat,{soruces={ss[1]}}) end),'unknown Question fields must be rejected')
  ok(not pcall(function() Query.new(world,pat,{sources={[2]=ss[1]}}) end),'sparse source sections must be rejected')
  ok(not pcall(function() Query.new(world,pat,{targets={[2]=d[1]}}) end),'sparse target sections must be rejected')
  ok(not pcall(function() Query.new(world,pat,{required={[2]=d[1]}}) end),'sparse required sections must be rejected')
  ok(not pcall(function() Query.new(world,pat,{admissible={[2]={from=ss[1],to=d[1]}}}) end),'sparse admissibility must be rejected')
  ok(not pcall(function() Query.new(world,pat,{seeds={[2]={from=ss[1],to=d[1]}}}) end),'sparse seeds must be rejected')
end
