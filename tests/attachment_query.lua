...
local W=require('worlds'); local Model,Att=W.Model,W.Attachment
local passed=0
...
test('1. retained query finds one witness without materialising a 60,480-witness space',function()
  local m,K,seed=permutation_problem(9,6); local q=Att.query(m,K,Att.patch(m,seed))
...
test('2. insufficient budget is Unknown; retained search later proves Retry without restart',function()
  local m,K,seed=permutation_problem(5,6); local q=Att.query(m,K,Att.patch(m,seed))
...
test('3. Retry certificate survives unrelated change and is invalidated by relevant supply',function()
  ...
  local q=Att.query(m,K,Att.patch(m,d)); local st,c=q:step(math.huge); eq(st,'retry'); assert(c:valid())
...
test('4. Point gluing invalidates Retry without moving authority and can make attachment possible',function()
  ...
  local p=Att.patch(m,d); local q=Att.query(m,K,p); local st,c=q:step(math.huge); eq(st,'retry'); assert(c:valid())
  m:glue_points(A,B); assert(not c:valid())
  local q2=Att.query(m,K,p); st=q2:step(math.huge); eq(st,'hit')
end)

test('5. a retained Unknown query restarts only when its relevant frontier changes',function()
  local m,K,seed=permutation_problem(5,6); local q=Att.query(m,K,Att.patch(m,seed)); eq(q:step(3),'unknown')
...
test('6. a stale witness cannot be grafted after its authority is consumed',function()
  ...
  local q=Att.query(m,K,Att.patch(m,d)); local st,w=q:step(math.huge); eq(st,'hit')
  local spent=m:strand('spent',K,{X},'Spent'); m:face('consume',K,{r},{spent},'Consume')
  local before=#m.objects; local ok=pcall(function() Att.graft(w) end); assert(not ok); eq(#m.objects,before,'stale graft partially mutated model')
end)
...
test('7. fixed Strand bindings cannot bypass exact-World locality',function()
  ...
  local q=Att.query(m,K1,Att.patch(m,d),{{demand=d,supply=foreign}}); local st=q:step(math.huge); eq(st,'retry')
end)

test('8. completed retained queries revalidate when their frontier changes',function()
  local m=Model.new('O'); local O=m.actuality; local X=m:point('X',O,'X')
  local D=m:world('D'); local G=m:world('G'); local d=m:strand('d',D,{X},'R'); local out=m:strand('out',G,{X},'S'); m:face('f',G,{d},{out},'F')
  local K=m:world('K',O); local p=Att.patch(m,d); local q=Att.query(m,K,p); local st=q:step(math.huge); eq(st,'retry')
...
test('9. Retry certificate validity is false rather than exceptional when its patch ceases to be attachable',function()
  local m=Model.new('O'); local O=m.actuality; local X=m:point('X',O,'X')
  local D=m:world('D'); local G=m:world('G'); local d=m:strand('d',D,{X},'R'); local out=m:strand('out',G,{X},'S'); m:face('f',G,{d},{out},'F')
  local K=m:world('K',O); local p=Att.patch(m,d); local q=Att.query(m,K,p); local st,c=q:step(math.huge); eq(st,'retry'); assert(c:valid())
