...
local function remap_cut(c,image)
    local r=cut()
    for i,s in ipairs(c.ingress) do r.ingress[i]=assert(image[s]) end
    for i,s in ipairs(c.egress) do r.egress[i]=assert(image[s]) end
    return r
end
local function append(dst,xs) for _,x in ipairs(xs or {}) do dst[#dst+1]=x end end

-- Compose arbitrary selected cuts by a same-Geometry Question over their disjoint
-- product. `edges` is a finite admissibility relation between concrete selected
-- egress/ingress occurrences. Nothing here knows Hole, Offer, Scalar or Context.
local function close_cuts(parts,cuts,edges,required)
    local product,image=W.join(parts,{})
    local mapped={}; local sources,targets={},{}
    for i,c in ipairs(cuts) do mapped[i]=remap_cut(c,image); append(sources,mapped[i].egress); append(targets,mapped[i].ingress) end
    local admissible={}; for _,e in ipairs(edges or {}) do admissible[#admissible+1]={from=assert(image[e.from]),to=assert(image[e.to])} end
    local req={}; if required then for _,s in ipairs(required) do req[#req+1]=assert(image[s]) end else append(req,targets) end
    local q=Q.close({product},{sources=sources,targets=targets,required_targets=req,admissible=admissible})
    local tag,evidence=decided(q); if tag=='no' then return tag,nil,evidence,product,image,mapped end
    local composite,cimage=W.join({product},evidence)
    return 'yes',composite,evidence,product,image,mapped,cimage
end
...
do
    ...
    local final_ap=cimage[image[ap]]; local final_bp=cimage[image[bp]]
...
do
    ...
    ok(cimage[image[ax.face]]==cimage[image[bx.face]],'both cyclic Faces must map to one joint Face')
end
...
do
    ...
    local tag2=close_cuts({src2,hand},{cuts2.control,cuts2.authority,hc.control,hc.authority},{
        {from=y.control,to=hx.control},{from=y.auth,to=hx.auth},
    })
    eq(tag2,'no','establishing the disjoint product first may make a later internal Handoff closure unlawful')

    -- The direct judgement is different: locality is established by the closure itself,
    -- not before it. This is exactly why Query.close(parts, ...) must exist.
    local q=Q.close({src2,hand},{
        sources={y.control,y.auth}, targets={hx.control,hx.auth},
        required_sources={y.control,y.auth}, required_targets={hx.control,hx.auth},
        admissible={{from=y.control,to=hx.control},{from=y.auth,to=hx.auth}},
    })
    local direct,es=decided(q); eq(direct,'yes','direct multi-part Handoff closure remains lawful')
    local g2=W.join({src2,hand},es); local live2=W.boundary(g2)
    ok(not live2:owns_strand(y.auth),'selected authority cut must consume caller occurrence')
...
do
    ...
    local product,image=W.join({src,dst},{})
    local question=Q.close({product},{
        sources={image[a],image[b]}, targets={image[x],image[y]}, required_targets={image[x]},
        admissible={{from=image[a],to=image[x]},{from=image[b],to=image[y]}},
    })
    local tag,es=decided(question); eq(tag,'yes'); eq(#es,1,'first exact witness should close only the required target')
    eq(es[1].from,image[a]); eq(es[1].to,image[x])
end
...
print('PASS Relay internal-cut/scalar Worlds toy',checks,'assertions')
